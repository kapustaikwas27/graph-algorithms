...
0 : 0
I came up with a non-recursive, linear way to compute 
level, pre and post order for any rooted tree using 
J features without performing depth-first search.
To make an examples of usage more understandable I used 
graphviz addon to visualize example trees which is 
available in Jqt.
...
NB. Enum to access elements from tree representation with names instead of numbers
...
NB. Postorder and preorder can be calculated similarly using sub-tree sizes. 
...
NB. row 1 is number of children of vertices from previous level,
...
'%d->%d;' show_tree_ e
NB. Tree representation of tree with weights of edges. 
...
NB. Weights of edges does not change orderings.
...
NB. y is height (number of edges from root to each leaf). y is non-negative integer.
genRegEdges =: ((<:@] $ [ # i.@:(>.@%~)) ,. }.@:i.@])"0

NB. Example of usages of genregedges.
